...
def random_quality():
    return random.choices(
        [75, 80, 85, 90, 95, 100], [0.05, 0.05, 0.15, 0.15, 0.20, 0.40]
    )[0]
...
def noisy(image):
    ...
    noise_typ = random.choices(
        ("gaussian", "s&p", "poisson", "speckle"), (0.3, 0.3, 0.3, 0.1)
    )[0]
...
def printStats(infos: List[List[ImageInfo]]) -> None:
    ...
    print(
        f"Images: {len(infos)}, median resolution: {median_res.width}x{median_res.height}, avg res: {avg_res:.1f}, median points: {median_points.n_points}, avg points: {avg_points:.1f}"
    )
