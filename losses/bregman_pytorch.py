...
def sinkhorn(a, b, C, reg=1e-1, method='sinkhorn', maxIter=1000, tau=1e3, stopThr=1e-9, verbose=False, log=True, warm_start=None, eval_freq=10, print_freq=200, **kwargs):
	...
	if method.lower() == 'sinkhorn':
		return sinkhorn_knopp(a, b, C, reg, maxIter=maxIter, stopThr=stopThr, verbose=verbose, log=log, warm_start=warm_start, eval_freq=eval_freq, **kwargs)
	else:
...
def sinkhorn_knopp(a, b, C, reg=1e-1, maxIter=1000, stopThr=1e-9, verbose=False, log=False, warm_start=None, eval_freq=10, **kwargs):
	...
	while (err > stopThr and it <= maxIter):
		...
		if torch.logical_or(torch.isnan(u), torch.isinf(u)).any() or torch.logical_or(torch.isnan(v), torch.isinf(v)).any():
