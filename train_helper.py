...
class Trainer(object):
    ...
    def setup(self):
        ...
        self.logger = log_utils.get_logger(
            os.path.join(self.save_dir, f"train-{time_str:s}.log")
        )
        ...
        train_subdir = (
            "train"
            if args.dataset.lower() in ("qnrf", "nwpu", "synth", "gcc")
            else "train_data"
        )
        test_subdir = (
            "val"
        ...
        self.datasets = {
            "train": Crowd(
                args.dataset.lower(),
                os.path.join(args.data_dir, train_subdir),
                args.crop_size,
                downsample_ratio,
                "train",
                args.mixed,
            ...
            "val": Crowd(
                args.dataset.lower(),
                os.path.join(args.data_dir, test_subdir),
                args.crop_size,
                downsample_ratio,
                "val",
                args.mixed,
    ...
    def train_epoch(self):
        ...
        for step, (inputs, points, gt_discrete) in enumerate(self.dataloaders["train"]):
            ...
            with torch.set_grad_enabled(True):
                ...
                # Compute OT loss.
                ot_loss, wd, ot_obj_value, cost = self.ot_loss(
    ...
    def val_epoch(self):
        ...
        self.logger.info(
            f"Epoch {self.epoch} Val, MSE: {mse:.2f} MAE: {mae:.2f}, Cost {time.time() - epoch_start:.1f} sec"
        )
        ...
        if (2.0 * mse + mae) < (2.0 * self.best_mse + self.best_mae):
            ...
            self.best_mae = mae
            self.logger.info(
                f"NEW BEST: mse {self.best_mse:.2f} mae {self.best_mae:.2f} model epoch {self.epoch}"
            )
            torch.save(
                model_state_dic,
                os.path.join(self.save_dir, f"best_model_{self.best_count}.pth"),
            )
            self.best_count += 1
